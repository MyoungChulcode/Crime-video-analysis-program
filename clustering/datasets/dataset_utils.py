...
def construct_net_input(vid_loader, channel_ext, spatial_transform,
    ...
    if modality:
        ...
        if choice == 'replace':
            ...
            channel_path = channel_paths[key]
            channel_loader = channel_ext[key][1]
            channel_clip = channel_loader(channel_path, frame_indices, choice, rgb_video_path=path)
            # image      =  videoloader  (path_optical, frame_indices)
            if spatial_transform is not None:
                channel_clip = [spatial_transform(img) for img in channel_clip]

            # input tensor (9) don't match with expected (3)
            # if key != 'salient' or key == 'salient' and torch.mean(torch.stack(channel_clip, 0)) >= SALIENT_MASK_THRESHOLD:
